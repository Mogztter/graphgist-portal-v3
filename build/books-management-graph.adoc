...
//IS_PART_OF
CREATE	 create (harry1)-[:IS_PART_OF]->(harryPotter)

//DESCRIBED_BY
CREATE	 create (harry1)-[:DESCRIBED_BY]->(magic),
		 create (harry1)-[:DESCRIBED_BY]->(students),
		 create (bookThief)-[:DESCRIBED_BY]->(students),
		 create (bookThief)-[:DESCRIBED_BY]->(death),
		 create (bookThief)-[:DESCRIBED_BY]->(judaism),
		 create (tom)-[:DESCRIBED_BY]->(boy),
		 create (tom)-[:DESCRIBED_BY]->(psychology),
		 create (tom)-[:DESCRIBED_BY]->(change)

//OF_TYPE
CREATE	 create (harry1)-[:OF_TYPE]->(fantasy),
		 create (harry1)-[:OF_TYPE]->(mystery),
		 create (harry1)-[:OF_TYPE]->(fiction),
		 create (bookThief)-[:OF_TYPE]->(novel),
		 create (bookThief)-[:OF_TYPE]->(historical),
		 create (bookThief)-[:OF_TYPE]->(fiction),
		 create (tom)-[:OF_TYPE]->(novel),
		 create (tom)-[:OF_TYPE]->(Bildungsroman),
		 create (tom)-[:OF_TYPE]->(childrensNovel)

//WRITTEN_BY
CREATE	 create (harry1)-[:WRITTEN_BY]->(rowling),
		 create (bookThief)-[:WRITTEN_BY]->(zusak),
		 create (tom)-[:WRITTEN_BY]->(twain)

//TRANSLATED_TO
CREATE	 create (harry1)-[:TRANSLATED_TO]->(harry1French),
		 create (harry1)-[:TRANSLATED_TO]->(harry1Polish),
		 create (harry1)-[:TRANSLATED_TO]->(harry1Spanish)

//HAS_ORIGINAL_LANGUAGE
CREATE	 create (harry1)-[:HAS_ORIGINAL_LANGUAGE]->(english),
		 create (bookThief)-[:HAS_ORIGINAL_LANGUAGE]->(english),
		 create (tom)-[:HAS_ORIGINAL_LANGUAGE]->(english)

//OF_LANGUAGE
CREATE	 create (harry1French)-[:OF_LANGUAGE]->(french),
		 create (harry1Polish)-[:OF_LANGUAGE]->(polish),
		 create (harry1Spanish)-[:OF_LANGUAGE]->(spanish)

//MADE_BY
CREATE	 create (harry1French)-[:MADE_BY]->(menard)

//MADE_INTO
CREATE	 create (harry1)-[:MADE_INTO]->(harry1Movie),
		 create (bookThief)-[:MADE_INTO]->(bookThiefMovie)

//WHEN_ACTION
CREATE	 create (harry1)-[:WHEN_ACTION]->(centuryXX),
		 create (bookThief)-[:WHEN_ACTION]->(centuryXX),
		 create (tom)-[:WHEN_ACTION]->(centuryXIX)

//WHEN_PUBLISHED
CREATE	 create (harry1)-[:WHEN_PUBLISHED]->(year1997),
		 create (bookThief)-[:WHEN_PUBLISHED]->(year2005),
		 create (tom)-[:WHEN_PUBLISHED]->(year1876)

//WHERE_ACTION
CREATE	 create (harry1)-[:WHERE_ACTION]->(London),
	     create (bookThief)-[:WHERE_ACTION]->(Germany),
	     create (tom)-[:WHERE_ACTION]->(US)

//WHERE_PUBLISHED
CREATE	 create (harry1)-[:WHERE_PUBLISHED]->(UK),
		 create (bookThief)-[:WHERE_PUBLISHED]->(Australia),
		 create (tom)-[:WHERE_PUBLISHED]->(US)

//PUBLISHED_BY
CREATE	 create (harry1)-[:PUBLISHED_BY]->(bloomsbury),
		 create (bookThief)-[:PUBLISHED_BY]->(picador),
		 create (tom)-[:PUBLISHED_BY]->(apc)
----
//graph
